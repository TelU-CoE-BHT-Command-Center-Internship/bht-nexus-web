...
const resultsCopy = {
  id: {
    ...
    noteRequiredLabel: "Tulis alasan sebelum memilih keputusan.",
    notePlaceholder: "Tulis alasan singkat untuk keputusan ini",
    requestFixLabel: "Minta perbaikan",
    resubmitLabel: "Kirim ulang untuk ditinjau",
    correctionTitle: "Perbaikan kandidat",
    beforeLabel: "Sebelum",
    afterLabel: "Sesudah",
    versionLabel: "Versi",
  ...
  en: {
    ...
    noteRequiredLabel: "Enter a reason before choosing a decision.",
    notePlaceholder: "Write a short reason for this decision",
    requestFixLabel: "Request changes",
    resubmitLabel: "Resubmit for review",
    correctionTitle: "Candidate correction",
    beforeLabel: "Before",
    afterLabel: "After",
  ...
  Omit<
    ...
    "candidates" | "decisionLabels" | "locale" | "sourceTabs"
  >
>;
...
const reviewCandidateSeeds = candidateSeeds.filter(
  (seed) =>
    seed.type !== "paper" &&
    seed.id !== "document-hibah" &&
    seed.status !== "completed",
);

function buildSourceTabs(locale: Locale): CandidateSourceTab[] {
  const all: CandidateSourceTab = {
    count: reviewCandidateSeeds.length,
    id: "all",
  ...
  return [
    ...
    ...sourceOrder
      .map((source) => ({
        count: reviewCandidateSeeds.filter((seed) => seed.source === source)
          .length,
        id: source,
...
export function getNexusScraperResultsContent(
  ...
  return {
    ...resultsCopy[locale],
    locale,
    candidates: reviewCandidateSeeds.map((seed) => {
    ...
    decisionLabels: {
      ...
      changes_requested: getDecisionLabel(locale, "changes_requested"),
