...
Model Administrasi memisahkan identitas akun, hubungan anggota opsional, role tingkat tinggi, dan status akses. `ACTIVE`, `INVITED`, serta `SUSPENDED` merupakan nilai mesin; label Indonesia berada pada lapisan presentasi. Alur undangan tidak menyimpulkan anggota dari email, tidak meminta password, dan hanya menampilkan profil anggota yang belum mempunyai hubungan akun pada adapter saat ini. Akun individual maupun operasional dapat sah tanpa anggota. Role hanya membawa label, deskripsi, dan ringkasan cakupan untuk membentuk UI; permission, data scope, token undangan, autentikasi, transaksi status, serta audit harus datang dari dan ditegakkan layanan server. Seluruh perubahan tetap berada pada state halaman dan kembali ke fixture setelah muat ulang penuh.
