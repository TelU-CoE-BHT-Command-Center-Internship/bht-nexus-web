...
| Monitoring KM | `getNexusMonitoringLandingData` yang memanggil `getNexusMonitoringRecords`, `measureIndicator`, `summarizeCategory`, `buildDomainView`, `getNexusMonitoringCategories`, `nexusMonitoringIndicatorProgress`, dan `nexusMonitoringUpdates` | membaca ulang rumah Data Resmi yang sama tanpa menyalinnya, menghitung realisasi setiap indikator terpantau dari kaitan KM eksplisit, menyusun sebaran per rumpun data, menghitung kemajuan indikator per domain, dan mengurutkan pembaruan rekam resmi terbaru |
