...
| Administrasi | `getNexusAdministrationContent` | daftar dan rincian akun, pencarian, filter status/role/hubungan anggota, undangan bertahap, role tingkat tinggi, serta tindakan akses sesuai status selama halaman aktif |
...
Model Anggota mengikuti kebutuhan identitas pada SRS: profil, status aktif/cuti/nonaktif, visibilitas publik, unit, bidang keahlian, dan pengenal eksternal dipisahkan dari akun login serta role/permission. Adapter menggunakan kembali nama, foto, penugasan, serta deskripsi yang sudah dipublikasikan pada halaman institusional. Tanggal bergabung, kontak personal, dan pengenal akademik yang belum tersedia dibiarkan kosong; alamat email umum CoE tidak disalin sebagai email personal setiap anggota, dan penugasan organisasi tidak diduplikasi sebagai bidang keahlian. Anggota baru dapat dicatat tanpa email atau foto; avatar inisial menjadi fallback dan visibilitas publik tidak aktif secara bawaan. Form tambah dan ubah memakai bentuk data serta validasi yang sama, termasuk normalisasi, format, dan keunikan lima pengenal akademik selama halaman aktif. Editor foto menyimpan sumber asli dan hasil crop sebagai dua nilai berbeda sehingga avatar dapat diatur ulang tanpa kehilangan komposisi awal. Anggota boleh belum mempunyai akun; pemeriksaan akses membedakan undangan akun baru, akun lama yang belum terhubung, hubungan yang sudah benar, dan konflik hubungan ke anggota lain. Perubahan form dan hubungan akun hanya hidup selama halaman aktif sampai endpoint anggota serta kebijakan akses server dihubungkan.

Model Administrasi memisahkan identitas akun, hubungan anggota, role tingkat tinggi, dan status akses. Satu direktori akun bersama memberi konteks awal yang sama kepada Administrasi dan alur akses pada Anggota, sedangkan pilihan serta referensi anggota selalu diturunkan dari `getNexusMembersContent` melalui ID, nama publik, dan penugasan kanonis. Relasi memakai union eksplisit `LINKED`, `NON_MEMBER`, `UNLINKED`, dan `CONFLICT`; kemiripan nama atau email tidak pernah menjadi keputusan identitas. `ACTIVE`, `INVITED`, serta `SUSPENDED` merupakan nilai mesin; label Indonesia berada pada lapisan presentasi. Konsep `accountKind` dihapus karena belum memiliki kontrak berwenang dan alur undangan tidak dapat mengelolanya secara koheren. Role hanya membawa label, deskripsi, dan ringkasan cakupan untuk membentuk UI; permission, data scope, token undangan, autentikasi, transaksi status, serta audit harus datang dari dan ditegakkan layanan server. Seluruh perubahan tetap berada pada state halaman dan kembali ke fixture setelah muat ulang penuh; tidak ada penyimpanan akun lokal lintas-route.

Tautan Administrasi menuju Anggota membawa `?member=<memberId>` dan halaman Anggota hanya memilih konteks tersebut apabila ID ada pada direktori kanonis; parameter tidak valid jatuh aman ke profil pertama. Loading dan error memakai file convention route Next.js, sedangkan no-access mengikuti kontrak kemampuan workspace. Dialog konfirmasi bersama menjaga fokus dan dipakai untuk draft undangan yang belum disimpan serta tindakan status yang memerlukan konfirmasi.
