...
| Monitoring KM | `getNexusMonitoringLandingData` yang memanggil `getNexusMonitoringRecords`, `measureIndicator`, `summarizeRiset`, `buildRisetView`, `getNexusMonitoringCategories`, `nexusMonitoringIndicatorProgress`, dan `nexusMonitoringUpdates` | membaca ulang rumah Data Resmi yang sama tanpa menyalinnya, menghitung realisasi indikator Riset dari kaitan KM eksplisit, menyusun sebaran per rumpun data, menghitung kemajuan indikator per domain, dan mengurutkan pembaruan rekam resmi terbaru |
