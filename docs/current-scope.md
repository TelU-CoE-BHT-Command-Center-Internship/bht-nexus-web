...
- Kanan atas Monitoring memuat pemilih periode, **Kelola target**, dan **Unduh laporan**; rincian indikator memuat pemilih periode, **Ubah target**, dan **Unduh rekam**. Pemilih periode menghitung ulang seluruh angka untuk periode terpilih dan membawanya pada alamat `?periode=`, termasuk tautan domain, indikator, dan breadcrumb. Periode yang belum terdaftar menampilkan keadaan belum tersedia beserta jalan kembali, dan pengelola dapat langsung menambahkannya.
...
- Unduh laporan menghasilkan CSV per periode dengan kolom yang mengikuti worksheet `Evaluasi`: target, TW1–TW4, belum terpetakan, realisasi, capaian, status, dan keterangan. Unduh rekam menghasilkan CSV rekam pembentuk satu indikator beserta alasan dihitung atau tidaknya. Berkas memakai pemisah titik koma dan BOM UTF-8 agar langsung terbaca Excel berlokal Indonesia; sel yang diawali tanda rumus diberi petik pengaman.
