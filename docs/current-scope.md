...
- Nilai realisasi dan catatan triwulan dari workbook tidak ditampilkan sebagai pembanding operasional dan tidak dipakai sebagai realisasi BHT Nexus. Jejak sumber migrasi tetap tersedia pada provenance rekam; realisasi operasional berasal dari Data Resmi. Pengelolaan periode, target tahunan, serta versi definisi berikutnya merupakan paket konfigurasi terpisah dan belum tersedia pada UI ini.
...
- Kemampuan membuka Monitoring KM tercatat pada katalog izin frontend kanonis sebagai modul `monitoring` dengan tindakan `view`, sehingga pemeriksaan route dan matriks izin Administrasi memakai satu pengertian yang sama. Penegakan akses tetap milik server; frontend hanya menyajikan keadaannya.
